...
Only updates records where ou_grade IS NULL AND over_2_5_odds IS NOT NULL.
...
def fetch_null_ou_grade_records(conn) -> pd.DataFrame:
    """Fetch records where ou_grade IS NULL and over_2_5_odds IS NOT NULL."""
    query = f"""
    SELECT 
        match_id,
    ...
    WHERE ou_grade IS NULL
      AND over_2_5_odds IS NOT NULL
    ORDER BY match_id
    """
    
    try:
        df = pd.read_sql(query, conn)
        logger.info(f"[OK] Fetched {len(df)} records with NULL ou_grade and non-NULL over_2_5_odds")
        return df
    except Exception as e:
