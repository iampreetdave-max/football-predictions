...
- Updates existing match_ids (overwrites all fields)
- Inserts new match_ids
...
# ==================== UPDATE COLUMNS (all except match_id) ====================
UPDATE_COLUMNS = [
    'date', 'league', 'league_name', 'home_id', 'away_id', 'home_team', 'away_team',
    'home_odds', 'away_odds', 'draw_odds', 'over_2_5_odds', 'under_2_5_odds',
    'ctmcl', 'predicted_home_goals', 'predicted_away_goals', 'confidence', 'grade', 'delta',
    'predicted_outcome', 'predicted_winner',
    'status', 'data_source', 'confidence_category',
...
def insert_to_table(table_name, db_data):
    """Insert new data and update existing data in a specific table"""
    ...
    try:
        ...
        return {'success': False, 'table': table_name, 'inserted': 0, 'updated': 0, 'errors': 0}
    ...
    existing_data = db_data[db_data['match_id'].isin(existing_ids)]
    new_data = db_data[~db_data['match_id'].isin(existing_ids)]

    print(f"\n  Records breakdown:")
    print(f"    • Total in CSV: {len(db_data)}")
    print(f"    • Already in {table_name} (will update): {len(existing_data)}")
    print(f"    • New to insert: {len(new_data)}")

    if len(db_data) == 0:
        print(f"\n✓ No records to process for {table_name}.")
        cursor.close()
        conn.close()
        return {'success': True, 'table': table_name, 'inserted': 0, 'updated': 0, 'errors': 0, 'skipped': True}
    ...
    if len(existing_data) > 0:
        print(f"\nUpdating {len(existing_data)} existing records in {table_name}...")
        ...
        for idx, row in existing_data.iterrows():
            try:
                # Values for SET clause (all columns except match_id) + match_id for WHERE
                update_values = [None if pd.isna(row[col]) else row[col] for col in UPDATE_COLUMNS]
                update_values.append(row['match_id'])  # WHERE match_id = %s

                cursor.execute(update_query, update_values)
                updated += 1

                if updated % 10 == 0:
                    print(f"  Progress: {updated}/{len(existing_data)} records updated...")

            except Exception as e:
                errors += 1
                error_msg = f"Match ID {row['match_id']} (update): {str(e)[:100]}"
    ...
    if len(new_data) > 0:
        ...
        for idx, row in new_data.iterrows():
            try:
                ...
                errors += 1
                error_msg = f"Match ID {row['match_id']} (insert): {str(e)[:100]}"
                error_details.append(error_msg)
    ...
    try:
        conn.commit()
        print(f"\n✓ Successfully committed to {table_name} ({inserted} inserted, {updated} updated)")
    except Exception as e:
        print(f"\n✗ Error committing to {table_name}: {e}")
        conn.rollback()
        cursor.close()
        conn.close()
        return {'success': False, 'table': table_name, 'inserted': 0, 'updated': 0, 'errors': len(db_data)}
...
for result in results:
    ...
    if result['success']:
        if result.get('skipped'):
            print(f"✓ {table_name}: No records to process")
        else:
            print(f"✓ {table_name}: {result['inserted']} inserted, {result['updated']} updated")
            if result['errors'] > 0:
                print(f"  ⚠️  {result['errors']} errors encountered (skipped)")
    else:
        print(f"✗ {table_name}: FAILED - 0 records processed")
